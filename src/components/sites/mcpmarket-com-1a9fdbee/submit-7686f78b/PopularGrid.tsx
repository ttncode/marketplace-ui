...
import type { PopularCard, PopularSection, SubmissionType } from "./submit-data";

const TOP_RANKS = 3;

/**
 * Same split as the leaderboards: the server card is a flex column; the skill card is a
 * block whose `h-full` body ignores the 6px dither strip and whose description keeps two lines.
 */
const LAYOUT: Record<SubmissionType, { card: string; body: string; description: string }> = {
...
// The leaderboards' card markup (not exported there), which is the source's `homepage_card` family.
function RankedCard({ card, variant }: { readonly card: PopularCard; readonly variant: SubmissionType }) {
  const layout = LAYOUT[variant];
  return (
    <Link id={card.id} href={card.href} className={cn("group block h-full", styles.link)}>
      <div
        className={cn(
      ...
      >
        ...
        <div className={cn("flex flex-col px-[19px] pt-[18px] pb-[17px]", layout.body)}>
          ...
          <p className={cn("mb-4 line-clamp-2 font-sans text-[13px] leading-[1.55] text-[#626262]", layout.description)}>
            {card.description}
          </p>
...
interface PopularGridProps {
  readonly section: PopularSection;
  readonly variant: SubmissionType;
  readonly hidden: boolean;
}

export function PopularGrid({ section, variant, hidden }: PopularGridProps) {
  return (
    <section aria-labelledby={section.headingId} hidden={hidden}>
      <div className="mb-8">
        <h2
          id={section.headingId}
          className="text-center font-display text-2xl leading-8 font-normal tracking-[-0.035em] text-foreground md:text-3xl md:leading-9"
        >
          {section.title}
        </h2>
        <p className="mt-2 text-center text-base leading-6 text-muted-foreground">{section.subtitle}</p>
      </div>
      <div className="space-y-8">
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {section.cards.map((card) => (
            <RankedCard key={card.id} card={card} variant={variant} />
          ))}
        </div>
        <div className="flex justify-center">
          <Link
            href={section.browse.href}
            className="inline-flex items-center text-sm leading-5 font-medium text-primary hover:underline"
          >
